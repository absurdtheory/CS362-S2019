 ...
 * Description: Tests the adventurer function in dominion.c
...
int main() {
    ...
    for (p = 0; p < numPlayer; p++)
    {
...
#if (NOISY_TEST == 1)
		/************************************************/
		/*TEST 1: Player's hand gains two treasure cards*/
		/************************************************/
		printf("\nTesting player %d's hand gains two treasure cards\n", p+1);
#endif
		...
		G.hand[p][G.handCount[p]] = adventurer;	//add adventurer card to hand
		G.handCount[p]++;
		...
		int prevHand[previousHandCount];		//holds cards in previous hand
		for (i = 0; i < previousHandCount; i++) {
			prevHand[i] = G.hand[p][i];
			if (prevHand[i] == copper || prevHand[i] == silver || prevHand[i] == gold) {	//calculate how much treasure is in hand before card is drawn
				prevTreasure++;
			}
		}
		...
		cardEffect(adventurer, choice1, choice2, choice3, &G, handpos, &bonus); 			//calls Adventurer card
		...
		int currHand[currentHandCount];		//holds cards in previous hand
		for (i = 0; i < currentHandCount; i++) {
			currHand[i] = G.hand[p][i];
			if (currHand[i] == copper || currHand[i] == silver || currHand[i] == gold) {	//calculate how much treasure is in hand after card is drawn
				currTreasure++;
			}
		}

		/*TEST 1a: cards added to player's hand*/
		if (currentHandCount - previousHandCount == 2 - 1) {		//if there is a gain of 2 cards minus one to represent adventurer card discarded
			i = p + 1;
			netChange = 0;
			while (i != p) {					//check if other player's hands were affected
				...
				if (netChange != 0) {
#if (NOISY_TEST == 1)
					printf("adventurerCard(): FAIL added card to other player's hand\n"); //if so, test fails
#endif
					allTestPass = 1;
					printPass = 1;
					break;
...
#if (NOISY_TEST == 1)
			printf("adventurerCard(): FAIL did not add two cards to current player's hand\n");
#endif
			printPass = 1;
			allTestPass = 1;
		}
	if (printPass == 0) {						//otherwise, test passes
			...
			printf("adventurerCard(): PASS added two card to current player's hand and adventurer card removed\n");
#endif
		}
#if (NOISY_TEST == 1)
		/*TEST 1b: cards added are treasure cards*/
		printf("Testing treasure cards added to player %d's hand\n", p+1);
#endif
		printPass = 0;
		if (currTreasure - prevTreasure!= 2) {
#if (NOISY_TEST == 1)
			printf("adventurerCard(): FAIL did not add two treasure cards to current player's hand\n");
#endif
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		else {
			printf("adventurerCard(): PASS added two treasure cards to current player's hand\n");
		...
		if (prevDeck - currDeck >= 2) {		//if two or more cards were removed from the player's deck
			i = p + 1;
			netChange = 0;
			while (i != p) {					//check if other player's decks piles were affected
				...
				if (netChange != 0) {
#if (NOISY_TEST == 1)
					printf("adventurerCard(): FAIL affected other players' decks \n"); //if so, test fails
#endif
					allTestPass = 1;
					printPass = 1;
					break;
...
#if (NOISY_TEST == 1)
			printf("adventurerCard(): FAIL deck did not change correctly\n");
#endif
			printPass = 1;
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		if (printPass == 0) {						//otherwise, test passes
			printf("adventurerCard(): PASS deck changed correctly\n");
		}
		/*********************************************************
		...
		*********************************************************/
		...
		if (currDiscard - prevDiscard == prevDeck - currDeck + 2 - 1) {		//if the discard pile has a gain of the difference between the previous and current deck counts minus the two treasure cards plus 1 for discarded adventurer card
			i = p + 1;
			netChange = 0;
			while (i != p) {					//check if other player's discard piles were affected
				...
				if (netChange != 0) {
#if (NOISY_TEST == 1)
					printf("adventurerCard(): FAIL affected other player's discard piles\n"); //if so, test fails
#endif
					allTestPass = 1;
					printPass = 1;
					break;
...
#if (NOISY_TEST == 1)
			printf("adventurerCard(): FAIL discard pile did not change correctly\n");
#endif
			printPass = 1;
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		if (printPass == 0) {						//otherwise, test passes
			printf("adventurerCard(): PASS discard pile changed correctly\n");
		}
		/*********************************************************
		*TEST 3: Adventurer card discarded
		*********************************************************/
		printf("Testing discard adventurer for player %d\n", p + 1);
#endif

		printPass = 0;
		if (G.hand[p][previousHandCount - 1] == adventurer) {
#if (NOISY_TEST == 1)
			printf("adventurerCard(): FAIL did not discard adventurer card\n");
#endif	
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		else {
			printf("adventurerCard(): PASS discard adventurer card\n");
		}
#endif
		/********************************************************/
...
#if (NOISY_TEST == 1)
			printf("adventurerCard(): PASS empty deck moved discard pile\n");
#endif	
			printPass = 0;
		}
		else {
#if (NOISY_TEST == 1)
			printf("adventurerCard(): FAIL empty deck did not move discard pile\n");
#endif	
			printPass = 1;
			allTestPass = 1;
		}
