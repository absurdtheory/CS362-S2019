 ...
 * Description: Randomly tests the council room card in dominion.c
...
#define DEBUG 0
#define NOISY_TEST 0	//set to 1 for printing all results, 0 to only print overall result
#define assertTrue(expr) if (!(expr)) {allTestPass = 1;}

void checkCouncilRoomCard(int p, int choice1, int choice2, int choice3, struct gameState *G, int handpos, int *bonus) {
  ...
  int prevBuys = G->numBuys;					//numBuys before cardEffect
  int i;
...
//get other players' deck count, discard count, and hand count before cardEffect
  ...
  int currBuys = G->numBuys;					//numBuys after cardEffect
  ...
  //check three net cards added to hand
  assertTrue(previousHandCount + 3 == currentHandCount);
#if NOISY_TEST == 1
  if (previousHandCount + 3 == currentHandCount) {
	  printf("councilRoomCard(): PASS net gain of 3 cards to hand\n");
  }
  else {
	  printf("councilRoomCard(): FAIL did not gain 3 cards to hand\n");
  }
#endif

  //check council room removed from hand
  assertTrue(G->hand[p][handpos] != council_room);
#if NOISY_TEST == 1
  if (G->hand[p][handpos] != council_room) {
	  printf("councilRoomCard(): PASS council room removed from hand\n");
  }
  else {
	  printf("councilRoomCard(): FAIL council room still in hand\n");
  }
#endif
  ...
  if (currDiscard - prevDiscard == 1) {
	  ...
	  printf("councilRoomCard(): FAIL discard pile did not gain 1 card\n");
  }
#endif

  //check deck removed 4 cards
  assertTrue(prevDeck - currDeck == 4);
#if NOISY_TEST == 1
  if (prevDeck - currDeck == 4) {
	  printf("councilRoomCard(): PASS 4 cards removed from deck\n");
  }
  else {
	  printf("councilRoomCard(): FAIL 4 cards not removed from deck\n");
  }
#endif

  //check buys increased by 1
  assertTrue(currBuys - prevBuys == 1);
#if NOISY_TEST == 1
  if (currBuys - prevBuys == 1) {
	  printf("councilRoomCard(): PASS 1 buy gained\n");
  }
  else {
	  printf("councilRoomCard(): FAIL did not gain 1 buy\n");
  }
#endif

  for (i = 0; i < G->numPlayers; i++) {
	  if (i != p) {
		  assertTrue(otherHandsCurr[i] - otherHandsPrev[i] == 1);   //check other players' gain 1 card to hand
#if NOISY_TEST == 1
		  if (otherHandsCurr[i] - otherHandsPrev[i] == 1) {
			  printf("councilRoomCard(): PASS other players gained 1 card to hand\n");
		  }
		  else {
			  printf("councilRoomCard(): FAIL other players did not gain 1 card to hand\n");
		  }
#endif
		  assertTrue(otherDecksPrev[i] - otherDecksCurr[i] == 1);   //check other players' deck removed 1 card
#if NOISY_TEST == 1
		  if (otherDecksPrev[i] - otherDecksCurr[i] == 1) {
			  printf("councilRoomCard(): PASS other players removed 1 card from deck\n");
		  }
		  else {
			  printf("councilRoomCard(): FAIL other players did not remove 1 card from deck\n");
		  }
#endif
		  ...
		  if (otherDiscardCurr[i] == otherDiscardPrev[i]) {
			  printf("councilRoomCard(): PASS other players discard did not changed\n");
		  }
		  else {
			  printf("councilRoomCard(): FAIL other players discard changed\n");
		  }
#endif
	  }
  }
...
int main () {
  ...
  for (n = 0; n < 50; n++) {
	...
	handpos = floor(Random() * G.handCount[p]);		//randomize position of council room in hand
	choice1 = floor(Random() * 26);					//randomize choice1
	choice2 = floor(Random() * 26);					//randomize choice2
	choice3 = floor(Random() * 26);					//randomize chsteoice3
	G.numBuys = floor(Random() * (INT_MAX - 1));	//randomize numBuys
	...
	G.hand[p][handpos] = council_room;				//put council room card at handpos

	checkCouncilRoomCard(p, choice1, choice2, choice3, &G, handpos, &bonus);	//call test				
  }
