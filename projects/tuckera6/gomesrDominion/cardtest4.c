...
int main() {
    ...
    for (p = 0; p < numPlayer; p++)
    {
		...
		G.hand[p][G.handCount[p]] = great_hall;	//add great_hall card to hand
		G.handCount[p]++;
		...
		cardEffect(great_hall, choice1, choice2, choice3, &G, handpos, &bonus); 			//calls Adventurer card
		...
		if (currentHandCount - previousHandCount == 0) {		//if there is a gain of 1 cards minus one to represent smithy card discarded
			...
			while (i != p) {					//check if other player's hands were affected
				...
				if (netChange != 0) {
#if (NOISY_TEST == 1)
					printf("greatHallCard(): FAIL added card to other player's hand\n"); //if so, test fails
#endif
					allTestPass = 1;
					printPass = 1;
					break;
...
#if (NOISY_TEST == 1)
			printf("greatHallCard(): FAIL did not add one card to current player's hand\n");
#endif
			printPass = 1;
			allTestPass = 1;
		}
...
#if (NOISY_TEST == 1)
			printf("greatHallCard(): PASS added one card to current player's hand and one card removed\n");
#endif
		}
#if (NOISY_TEST == 1)
			/*****************************************************************/
		...
		if (prevDeck - currDeck == 1) {		//if one card removed from deck
			...
			while (i != p) {					//check if other player's decks piles were affected
				...
				if (netChange != 0) {
#if (NOISY_TEST == 1)
					printf("greatHallCard(): FAIL affected other players' decks \n"); //if so, test fails
#endif
					allTestPass = 1;
					printPass = 1;
					break;
...
#if (NOISY_TEST == 1)
			printf("greatHallCard(): FAIL deck did not change correctly\n");
#endif
			printPass = 1;
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		if (printPass == 0) {						//otherwise, test passes
			printf("greatHallCard(): PASS deck changed correctly\n");
		}
		/*********************************************************
		...
		*********************************************************/
		...
		if (currDiscard - prevDiscard == 1) {		//if the discard pile has a gain of one card
			...
			while (i != p) {					//check if other player's discard piles were affected
				...
				if (netChange != 0) {
#if (NOISY_TEST == 1)
					printf("greatHallCard(): FAIL affected other player's discard piles\n"); //if so, test fails
#endif
					allTestPass = 1;
					printPass = 1;
					break;
...
#if (NOISY_TEST == 1)
			printf("greatHallCard(): FAIL discard pile did not change correctly\n");
#endif
			printPass = 1;
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		if (printPass == 0) {						//otherwise, test passes
			printf("greatHallCard(): PASS discard pile changed correctly\n");
		}
		/*********************************************************
		...
		*********************************************************/
		...
		printPass = 0;
		if (G.hand[p][previousHandCount - 1] == great_hall) {
#if (NOISY_TEST == 1)
			printf("greatHallCard(): FAIL did not discard adventurer card\n");
#endif	
			allTestPass = 1;
		}
#if (NOISY_TEST == 1)
		else {
			printf("greatHallCard(): PASS discarded great hall card\n");
		}
#endif
		/********************************************************/
...
#if (NOISY_TEST == 1)
			printf("greatHallCard(): PASS empty deck moved discard pile\n");
#endif	
			printPass = 0;
		}
		else {
#if (NOISY_TEST == 1)
			printf("greatHallCard(): FAIL empty deck did not move discard pile\n");
#endif	
			printPass = 1;
			allTestPass = 1;
		}
		...
		/********************************************************/
		/*TEST 4: Number of actions increased by one*/
		/********************************************************/
		if (currActions - prevActions == 1) {
#if (NOISY_TEST == 1)
			printf("greatHallCard(): PASS player gained one action\n");
#endif	
		}
		else {
#if (NOISY_TEST == 1)
			printf("greatHallCard(): FAIL player did not gain one action\n");
#endif	
			allTestPass = 1;
		}
