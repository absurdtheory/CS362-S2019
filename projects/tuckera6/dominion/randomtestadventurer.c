 ...
 * Description: Randomly tests the adventurer function in dominion.c
...
void checkAdventurerCard(int p, int choice1, int choice2, int choice3, struct gameState *G, int handpos, int *bonus) {
  ...
  //treasure in hand before cardEffect
 int prevTreasure = 0;
  for (i = 0; i < previousHandCount; i++) {
	  if (G->hand[p][i] == copper || G->hand[p][i] == silver || G->hand[p][i] == gold) {	//calculate how much treasure is in hand before card is drawn
		  prevTreasure++;
	  }
  }
  ...
  //treasure in hand after cardEffect
  int currTreasure = 0;
  for (i = 0; i < currentHandCount; i++) {
	  if (G->hand[p][i] == copper || G->hand[p][i] == silver || G->hand[p][i] == gold) {	//calculate how much treasure is in hand after card is drawn
		  currTreasure++;
	  }
  }
  ...
  //check two treasure cards added to hand
  assertTrue(prevTreasure + 2 == currTreasure);
#if NOISY_TEST == 1
  if (prevTreasure + 2 == currTreasure) {
	  printf("adventurerCard(): PASS two treasures gained to hand\n");
  }
  else {
	  printf("adventurerCard(): FAIL two treasures not added to hand\n");
  }
#endif

  //check adventurer removed from hand
  assertTrue(G->hand[p][handpos] != adventurer);
#if NOISY_TEST == 1
  if (G->hand[p][handpos] != adventurer) {
	  printf("adventurerCard(): PASS adventurer removed from hand\n");
  }
  else {
	  printf("adventurerCard(): FAIL adventurer still in hand\n");
  }
#endif

  //check discard pile gained at least 1 card
  assertTrue(currDiscard - prevDiscard >= 1);
#if NOISY_TEST == 1
  if (currDiscard - prevDiscard >= 1) {
	  printf("adventurerCard(): PASS at least 1 card added to discard\n");
  }
  else {
	  printf("adventurerCard(): FAIL no cards added to discard\n");
  }
#endif

  //check deck removed at least 2 cards
  assertTrue(prevDeck - currDeck >= 2);
#if NOISY_TEST == 1
  if (prevDeck - currDeck >= 2) {
	  printf("adventurerCard(): PASS at least two cards removed from deck\n");
  }
  else {
	  printf("adventurerCard(): FAIL less than two cards removed from deck\n");
  }
#endif
 
  for (i = 0; i < G->numPlayers; i++) {
	  if (i != p) {
		  assertTrue(otherHandsCurr[i] == otherHandsPrev[i]);   //check other players' hand did not change
#if NOISY_TEST == 1
		  if (otherHandsCurr[i] == otherHandsPrev[i]) {
			  printf("adventurerCard(): PASS other players' hands unchanged\n");
		  }
		  else {
			  printf("adventurerCard(): FAIL other players' hands changed\n");
		  }
#endif
		  assertTrue(otherDecksPrev[i] == otherDecksCurr[i]);   //check other players' deck did not change
#if NOISY_TEST == 1
		  if (otherDecksPrev[i] == otherDecksCurr[i]) {
			  printf("adventurerCard(): PASS other players' decks unchanged\n");
		  }
		  else {
			  printf("adventurerCard(): FAIL other players' decks changed\n");
		  }
#endif
		  ...
		  if (otherDiscardCurr[i] == otherDiscardPrev[i]) {
			  printf("adventurerCard(): PASS other players' discard unchanged\n");
		  }
		  else {
			  printf("adventurerCard(): FAIL other players' discard changed\n");
		  }
#endif
	  }
  }
...
int main () {
  ...
  for (n = 0; n < 50; n++) {
	...
	handpos = floor(Random() * G.handCount[p]);		//randomize position of Adventurer in hand
	...
	G.hand[p][handpos] = adventurer;				//put adventurer card at handpos

	checkAdventurerCard(p, choice1, choice2, choice3, &G, handpos, &bonus);	//call test				
  }
